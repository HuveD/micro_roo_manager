...
3.  **Delegate & Verify Loop:**
    ...
    c.  Gather necessary context **as specified or implied by the original request and any relevant information from previous steps (e.g., analysis results, file paths, document references)**.
    d.  Use the built-in `new_task` tool to delegate the subtask, adhering strictly to `subtask_protocol.md`. **Ensure the `## CONTEXT` section is comprehensive.**
