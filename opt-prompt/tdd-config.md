...
5.  **Verify:** After report, use `execute_command` (**FULL SUITE PASS** - refer to `docs/rules/tool_guide.md`) & `read_file` (structure improved, spec adherence maintained). **If full suite fails, trigger [Specification Review Workflow].**

## 5. Specification Review Workflow (Mandatory on Full Suite Failure)
If triggered by **any** full suite verification failure:
1.  **Identify Failures:** Analyze `execute_command` output (refer to `docs/rules/tool_guide.md` for command) for failing tests.
...
3.  **Verify Fix & Full Suite:** After `code` report, **re-run the FULL test suite** (`execute_command` with full suite option - refer to `docs/rules/tool_guide.md`). Verify **ALL tests pass**. Verify fix aligns with spec doc (`read_file`).
...
*   ✅ **FULL Test Suite Verified & Passed:** Final **full suite** tests run (`execute_command` - refer to `docs/rules/tool_guide.md`) and **confirmed to pass (including any spec-driven regression fixes)**.
...
*   **Verification:** Use `execute_command` (**run FULL test suite** - refer to `docs/rules/tool_guide.md`), `read_file` (verify code against **specific specification document**, conventions, support spec review).
