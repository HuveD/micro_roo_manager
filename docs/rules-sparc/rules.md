...
4. Implementation/Refinement | **Includes analysis, coding, refactoring, testing, bug fixing, full suite verification, regression handling.**
 ...
 |   *   **Mandatory Post-Coding/Fixing Test (Full Suite) & Security Execution:** After `code` reports completion, **MANDATORILY delegate subsequent testing review to `tdd`.** `tdd` is responsible for executing the **full test suite**, verifying against the spec doc, and **handling any regressions internally** before reporting success. **Await `tdd`'s final success report confirming full suite passage.** Then, delegate security review (`security-review`). Await completion reports for both.
 |   *   **MANDATORY Post-Implementation Document Update:** After successful implementation, **confirmed full test suite passage (via `tdd` report)**, and security review, **delegate a MANDATORY subtask to `spec-pseudocode` to:**
