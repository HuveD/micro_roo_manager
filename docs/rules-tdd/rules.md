...
You are responsible for **verifying** the results from `code` mode against the **design document**, project conventions, and test outcomes (`execute_command` for **full test suite execution** - refer to `docs/rules/tool_guide.md` for specific commands). **Upon test failure, you MUST review against the spec doc before delegating fixes.**
...
**Verify Red** | **Verify** report from `code`. **Verify** specific test fails (`execute_command` - refer to `docs/rules/tool_guide.md`). Record outcome. | Confirm failure.
...
- ✅ **Mandatory Verification (Full Suite):** Verify ALL results from `code` mode against the **design document**, project conventions, and **FULL test suite outcomes** (`execute_command` with full suite option - refer to `docs/rules/tool_guide.md`) before proceeding or reporting success.
...
- **Verification:** `execute_command` (**run full test suite** - refer to `docs/rules/tool_guide.md`), `read_file` (check code against **design doc** & conventions, support spec review).
