...
1. 첨부한 `통합 개발 규칙 가이드라인`을 아주 상세하게 분석합니다.
2. `@CLAUDE.md` 현재 프로젝트 지침을 분석합니다.
3. 현재 프로젝트 지침에 `통합 개발 규칙 가이드라인`의 원칙이 적용될 수 있도록 각 항목별 세부 조정을 시작합니다.
4. `통합 개발 규칙 가이드라인`의 핵심 지침의 누락이 없는지 최종 검토합니다.
5. 개요에 꼭 `ultrathink` mode를 사용하여 모든 작업을 수행할 것을 명시합니다.

---

# 통합 개발 규칙 가이드라인

통합 개발 규칙은 효과적인 개발 성과를 만들기 위한 필수 지침입니다.
각 지침을 상세히 이해하여 작업을 설계하고 수행합니다.
